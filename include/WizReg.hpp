...
//don't use these directly - use GetSocketRegAddress()
struct SockReg final
{
	enum Register
	{
	...
	static uint16_t GetSocketRegAddress(SockReg::Register reg, uint8_t socketId)
	{
...
struct SockMode final //mode register
{
	enum Mode
	{
...
struct SockCommand final //command register
{
	enum Command
	{
...
struct SockStatus final //status register
{
	enum Status
	{	
